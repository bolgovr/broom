...
Broom.prototype.extend = function (flow) {
  if (flow instanceof Broom) {
    var prop;
    for (prop in flow.flowTree) {
      if (!this.flowTree.hasOwnProperty(prop)) {
        this.flowTree[prop] = flow.flowTree[prop];
      }
    }
    return true;
  } else {
